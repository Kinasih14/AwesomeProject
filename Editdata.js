...
const Createdata = () => {
    const jsonUrl = `http://192.168.100.227:3000/mahasiswa`; //API yg digunakan emulator untuk akses localhost komputer
    ...
    const submit = () => {
        ...
        fetch(`http://192.168.100.227:3000/mahasiswa/${selectedUser.id}`, {
            method: 'PUT',
            headers: {
        ...
        <View>
          {isLoading ? (
            ...
            <View>
             ...
             renderItem={({ item }) => (
               <View>
                 <TouchableOpacity style={styles.button} onPress={() => selectItem(item)}>
